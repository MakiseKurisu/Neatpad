...
void SetWindowFileName(HWND hwnd, LPTSTR szFileName, BOOL fModified)
{
    ...
    wsprintf(ach, TEXT("%s - %s%s"), szFileName, g_szAppName, mod);
...
LRESULT CALLBACK AboutDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFY:
        ...
        case NM_CLICK: case NM_RETURN:
            ShellExecute(hwnd, TEXT("open"), WEBSITE_URL, 0, 0, SW_SHOWNORMAL);
            return 0;
        }
...
UINT TextViewNotifyHandler(HWND hwnd, NMHDR *nmhdr)
{
    ...
    case TVN_CURSOR_CHANGE:
        ...
        SetStatusBarText(g_hwndStatusbar, 1, 0, TEXT(" Ln %d, Col %d"),
...
UINT CommandHandler(HWND hwnd, UINT nCtrlId, UINT nCtrlCode, HWND hwndFrom)
{
    ...
    case IDM_FILE_NEW:
        ...
        SetWindowFileName(hwnd, TEXT("Untitled"), FALSE);
    ...
    case IDM_FILE_SAVE:
        MessageBox(hwnd, TEXT("Not implemented"), APP_TITLE, MB_ICONINFORMATION);
        return 0;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CLOSE:
        ...
        {
            UINT r;
            wsprintf(msgstr, TEXT("Do you want to save changes to\r\n%s?"), g_szFileName);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmdLine, int iShowCmd)
{
    ...
    if (pszCmdline && *pszCmdline == '-')
    {
        ...
        if (lstrcmpi(arg, TEXT("-uac")) == 0)
        {
            //return 0;
        }
        // image-file-execute-options
        else if (lstrcmpi(arg, TEXT("-ifeo")) == 0)
        {
            // skip notepad.exe
            pszCmdline = GetArg(pszCmdline, arg, MAX_PATH);
        }
    ...
    if (argv && argc == 4 && lstrcmpi(argv[1], TEXT("-uac")) == 0)
    {
